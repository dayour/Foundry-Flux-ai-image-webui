...
const ModelSettingsDialog = ({
    ...
    // UI modes: quick = compact, pro = fully expanded
    const [mode, setMode] = useState<"quick" | "pro">("quick");
    // Local toggle simulating Pro entitlement; when false, pro-only controls are greyed-out
    ...
    const handleDelete = async (model: GeneratorModelOption) => {
        ...
        try {
            ...
            let result: DeleteResponse = null;
            try {
    ...
    return (
        <Dialog.Root open={open} onOpenChange={onOpenChange}>
            <Dialog.Portal>
                ...
                <Dialog.Content
                    className="fixed left-1/2 top-1/2 z-[60] w-[92vw] max-w-3xl -translate-x-1/2 -translate-y-1/2 rounded-xl bg-white shadow-xl focus:outline-none dark:bg-neutral-900"
                    aria-modal="true"
                    role="dialog"
                    id="model-settings-dialog"
                >
                    {/* Constrain inner content height so dialog never overflows viewport
                        and allow internal scrolling for long forms / lists */}
                    <div className="max-h-[90vh] overflow-hidden rounded-xl">
                        <div className="p-6 overflow-y-auto max-h-[72vh] text-gray-900 dark:text-neutral-100">
                    <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            <div className="flex items-center gap-2 rounded-full bg-gray-100 px-2 py-1 text-xs dark:bg-neutral-800">
                                <button
                                    onClick={() => setMode("quick")}
                                    className={`px-2 py-1 rounded-full transition ${mode === "quick" ? "bg-white dark:bg-neutral-900 shadow-sm font-semibold" : "text-gray-600 dark:text-neutral-400"}`}
                                >
                                    Quick
                                </button>
                                <button
                                    onClick={() => setMode("pro")}
                                    className={`px-2 py-1 rounded-full transition ${mode === "pro" ? "bg-white dark:bg-neutral-900 shadow-sm font-semibold" : "text-gray-600 dark:text-neutral-400"}`}
                                >
                                    Pro
                                </button>
                            </div>
                            <button
                                onClick={() => setProEnabled((v) => !v)}
                                title={proEnabled ? "Pro enabled" : "Enable Pro features"}
                                className={`inline-flex items-center gap-2 rounded-md px-2 py-1 text-xs font-medium transition ${proEnabled ? "bg-amber-50 text-amber-700 shadow-sm" : "bg-gray-100 text-gray-700 dark:bg-neutral-800"}`}
                            >
                                <SparkleIcon className="h-4 w-4" />
                                {proEnabled ? "Pro" : "Pro off"}
                            </button>
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="mt-3 flex flex-wrap gap-2">
                                                    ...
                                                    <button
                                                        type="button"
                                                        className={`inline-flex items-center gap-1 rounded-md border px-3 py-1.5 text-xs font-medium transition ${proEnabled ? "border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100" : "border-gray-200 bg-transparent text-gray-400 cursor-not-allowed"}`}
                                                        disabled={!proEnabled || isBusy}
                                                        title={!proEnabled ? "Pro required" : "Edit advanced model"}
                                                    >
                        ...
                        <section>
                            ...
                            >
                                ...
                                {/* In quick mode hide pro-only helper fields */}
                                {mode === "pro" && (
                                    <div className="flex items-center gap-2 text-xs">
                                        <span className="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2 py-1 text-gray-600">Tip: Pro mode exposes advanced quality options</span>
                                    </div>
                                )}
                                <div className="grid gap-2">
                                    <button
                                        type="submit"
                                        className="inline-flex flex-1 items-center justify-center gap-2 rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold text-white transition hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60"
                                        disabled={creating}
                                    >
                                        {creating ? (
                                    ...
                                    <button
                                        ...
                                        className="inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-sm font-medium text-gray-700 dark:text-neutral-200"
                                    >
                                        Reset
                                    </button>
                                 </div>
